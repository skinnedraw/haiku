...
void render_t::render_page_content(float group_width)
{
    ...
    else if (m_iCurrentPage == ImPage_Movement)
    {
        ImGui::BeginGroup();
        {
            begin_child_styled("Movement", ImVec2(group_width, ImGui::GetFontSize() * 9.0f + style.ItemSpacing.y * 6 + style.WindowPadding.y * 2));
            {
                ...
                checkbox_hover("Tickrate", &settings::tickrate::enabled);
                ImGui::SameLine();
                keybind_button("TickrateKey", &settings::tickrate::keybind, &settings::tickrate::keybind_mode, 0);
                ImGui::PushItemWidth(ImGui::GetWindowWidth() - style.WindowPadding.x * 2);
                ImAdd::SliderInt("Tickrate", &settings::tickrate::value, 1, 660, "%d");
                ImGui::PopItemWidth();
